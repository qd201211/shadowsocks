...
BBR_installation_status(){
    ...
    #!更改bbr下载地址
		...
		#!更改本地模式if ! wget -N --no-check-certificate http://myktw.cn:10003/get/bbr.sh; then
		#!更改本地模式	echo -e "${Error} BBR 脚本下载失败 !" && exit 1
		#!更改本地模式else
		#!更改本地模式	echo -e "${Info} BBR 脚本下载完成 !"
		chmod +x bbr.sh
		fi
}
# 设置 防火墙规则
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 7777):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="7777"
...
Set_config_password(){
	...
	stty erase '^H' && read -p "(默认: myktw777):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="myktw777"
...
# 更改shadowsocksr下载地址
	...
	#!更改本地模式[[ ! -e "manyuser.zip" ]] && echo -e "${Error} ShadowsocksR服务端 压缩包 下载失败 !" && rm -rf manyuser.zip && exit 1
	#!取消解压unzip "manyuser.zip"
	#!取消判定[[ ! -e "/usr/local/shadowsocksr-manyuser/" ]] && echo -e "${Error} ShadowsocksR服务端 解压失败 !" && rm -rf manyuser.zip && exit 1
