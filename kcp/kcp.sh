...
apt update
	...
	if [[ -z ${screen_ver} ]]; then
		echo -e "${Info} 没有安装screen，开始安装..."
		apt-get install -y screen
	fi
	
	zip_ver=`zip -h`
	if [[ -z ${zip} ]]; then
		echo -e "${Info} 没有安装zip，开始安装..."
...
EOF
cat > client/start.udp.sh <<-EOF
#!/bin/bash
./udp2raw_amd64 -c -l0.0.0.0:$localprot -r $remoteip:$localprot -k 84RftO0agXxUwB4X  --raw-mode faketcp -a
EOF
#*客户端配置
